...
cmd({
  ...
  try {
    ...
    const collected = await conn.awaitMessage(filter, 60000); // Wait max 60s

    if (!collected) return reply("⏱️ Time out. Please try again.");

    const receivedText = collected.message?.conversation || collected.message?.extendedTextMessage?.text;

    await conn.sendMessage(from, {
      react: { text: '⬇️', key: collected.key }
    });
...
cmd({
  ...
  try {
    ...
    const apiUrl = `https://api.fgmods.xyz/api/downloader/gdrive?url=${q}&apikey=fg_8qWFaSmK`;
    const response = await axios.get(apiUrl);
...
const safeFileName = `KAVI-MD ${originalName.replace(/[\\/:*?"<>|]/g, '_')}`;
